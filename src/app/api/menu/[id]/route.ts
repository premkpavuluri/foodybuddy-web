...
export async function GET(
  ...
  try {
    Logger.info('BFF: Fetching menu item by ID', {
      itemId: id,
    ...
    if (!item) {
      Logger.warn('BFF: Menu item not found', {
        itemId: id,
    ...
    Logger.info('BFF: Successfully fetched menu item', {
      itemId: id,
      itemName: item.name,
      responseTime: `${responseTime}ms`
    })
    ...
    const responseTime = Date.now() - startTime
    Logger.error('BFF: Error fetching menu item', {
      itemId: id,
